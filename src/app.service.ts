  ...
  sendMessage(body: MessageDto, res: Response) {
    ...
    switch (body.eventType) {
      case "contact support":
        return res.status(HttpStatus.OK).send({ sucess: true, message: "Entrez votre nom svp", haveFeedBack: false });
      case "userName":
        ticketCreation.userName = body.message;
        return res.status(HttpStatus.OK).send({ sucess: true, message: "Entrez votre email svp", haveFeedBack: false });
      case "email":
        ticketCreation.email = body.message;
        return res.status(HttpStatus.OK).send({ sucess: true, message: "Veillez entrez une description claire du probleme que vous rencontrez", haveFeedBack: false });
      case "description":
